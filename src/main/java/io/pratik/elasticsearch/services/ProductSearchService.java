...
public class ProductSearchService {
	...
	public List<Product> processSearch2(final String name) {
		log.info("Search with query {}", name);
		QueryBuilder queryBuilder =
				QueryBuilders.boolQuery().must(QueryBuilders.matchQuery("name",name));
		Query searchQuery = new NativeSearchQueryBuilder()
				.withFilter(queryBuilder)
				.build();
		SearchHits<Product> productHits =
	...
	public List<Product> fetchSuggestions(String query) {
		QueryBuilder queryBuilder = QueryBuilders
